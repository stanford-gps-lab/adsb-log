...
% See Also: adsblog.FlightSegment, adsblog.Sighting
    ...
    methods
        ...
        function ac = getAircraftByLocation(obj, lat, lon, varargin)
            % getAircraftByLocation     retrieve a subset of the aircraft
            % in the list that are close to a given location.
            %   ac = aircraft.getAircraftByLocation(lat, lon) finds all the
            %   aircraft that pass within 2km horizontally and 500m
            %   vertically of the location specified as lat, lon in
            %   degrees.
            %
            %   ac = aircraft.getAircraftByLocation(..., 'ParamName',
            %   'ParamValue') allows specifying specific data in the name,
            %   value pairs.  The parameter names are as follows:
            %       - 'HorizontalDistance' - specify the horizontal range
            %       to consider an aircraft close to the location in [m]
            %       (default is 2000m)
            %
            %       - 'VerticalDistance' - specify the vertical distance to
            %       consider an aircraft close to the location in [m]
            %       (default is 500m)
